...
public class DanhSachPhim {
	...
	public Phim searchMa(String x) {
		for (Phim i : dsPhim) {
			if (i.getMaPhim().compareToIgnoreCase(x) == 0)
				return i;
		}
		return null;
	}
	...
	public String xuatHetTheoNgay(LocalDate a) {
		...
		str += "Phim noi: \n" + PhimNoi.tieuDe() + "\n";
		for (Phim i : dsPhim) {
			if (i instanceof PhimNoi && i.getNgayChieu().isAfter(a) == true)
				str += i + "\n";
		}
		str += "Phim Ngoai: \n" + PhimNgoai.tieuDe() + "\n";
		for (Phim i : dsPhim) {
			if (i instanceof PhimNgoai && i.getNgayChieu().isAfter(a) == true)
				str += i + "\n";
		}
		return str;
	}
